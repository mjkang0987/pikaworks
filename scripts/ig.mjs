...
function slideBody(d, t, index) {
  ...
  if (d.template === 'list') {
    ...
    // 저장된 클립 목록. 실제 목록 한 줄의 구조(썸네일 · 제목 · 도메인 · 태그)를
    // 그대로 따른다. 썸네일은 대표 이미지가 없을 때 채워지는 그라디언트다 —
    // 여기서 보여주려는 건 이미지가 아니라 제목이 읽힌다는 것이라 그대로 둔다.
...
function css(t, fontUrl) {
  ...
  * { margin:0; padding:0; box-sizing:border-box; }
  ...
  /* ── 클립 목록 ── 저장해둔 것들이 목록에서 어떻게 보이는지.
     한 줄이 썸네일·제목·도메인·태그로 끝나서, 훑기만 해도 무엇인지 읽힌다는
     말을 그림으로 그대로 옮긴 것이다. 태그는 켜짐(키컬러)/꺼짐(테두리) 둘뿐이라
     진행 중과 마감이 색 하나로 갈린다. */
