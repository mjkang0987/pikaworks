...
3. [x] **`clipnote-5`** — 여행 준비 중 카페・인스타・블로그에 흩어진 링크를
   「제주」 태그로 모으기. 캡션은 사용자가 직접 줬다. 이미지는 여러 차례
   검수·재작업을 거쳤다 (재탕 지적으로 source·clips 템플릿 교체, 실사
   스크린샷 크기·크롭 조정, 공감형 질문 카드 추가, 저장 결과 카드에
   숙소·여행지·맛집 3건+태그 추가) — 최종 6장(표지·질문·자동입력·저장결과·
   태그모으기·마감). 2026-09-02 사용자가 캡션 "없어", 이미지 "ok" 로 검수
   완료해 `status: image_ok`. 다음은 `/sns-queue` 로 발행일 배정.
...
4. **`/sns-queue` 로 `clipnote-5` 발행일 배정** 대기 중
