...
3. **`clipnote-5` 이미지 검수 대기** — 사용자가 캡션(여행 준비 중 카페・인스타・
   블로그에 흩어진 링크를 「제주」 태그로 모으기)을 직접 주고 이미지 제작을
   지시해 `status: design_done` 까지 만들어뒀다 (이미지 4장,
   `public/ig/clipnote-5-*.png`). `image_ok` 는 사람만 쓸 수 있어 다음 단계는
   사람 검수 → 통과하면 `/sns-queue` 로 발행일 배정이다.
   포지셔닝 문서(`pipeline/positioning.md`)의 "타겟 아님 — 링크를 모으는
   사람"과 이번 캡션(순수 저장·태그 아카이빙)이 부딪히는 것 같아 확인했고,
   저장·태그 기능도 처음부터 그 사람들을 위해 만든 것이라는 답을 받아 원문
   그대로 진행했다 — 문서와 실제 마케팅 관행 사이의 이 간극은 다음에
   `positioning.md` 를 손볼 때 반영할 만하다
